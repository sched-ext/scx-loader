...
impl SchedState {
    /// Getter representation, diff order; the single source for signal dictionaries.
...
/// Everything a transition mutates, grouped under one owner so a single
/// lock can guard it as a unit.
struct LoaderState {
    sched: SchedState,
...
impl LoaderState {
    ...
    async fn apply_and_signal(&mut self, emitter: &SignalEmitter<'_>, new: SchedState) {
        let changed = self.sched.diff(&new);
        self.sched = new;
...
mod tests {
    ...
    fn diff_transition_table() {
        ...
        // Mode run → args run, same scheduler: mode resets, args appear.
        assert_eq!(
            gaming.diff(&custom),
            ChangedProps {
                scheduler: false,
        ...
        // `None` and `Some([])` both read as an empty array — no fake change.
    ...
    /// Pins `property_values` to the declared names, signatures and empty-state values.
    #[test]
